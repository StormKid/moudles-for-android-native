package com.stormkid.litepal.pinyin;
...
public final class CNPinyinFactory {
    ...
    public static <T extends CN> ArrayList<CNPinyin<T>> createCNPinyinList(List<T> tList) {
        if (tList != null && !tList.isEmpty()) {
            ...
            for (int i=0;i<cnPinyinArrayList.size();i++){
                CNPinyin<T> tcnPinyin = cnPinyinArrayList.get(i);
                SURNAMES.put(tcnPinyin.getFirstChar(), tcnPinyin.firstChars);
            }
    ...
    private static <T extends CN> CNPinyin<T> createCNPinyin(T t) {

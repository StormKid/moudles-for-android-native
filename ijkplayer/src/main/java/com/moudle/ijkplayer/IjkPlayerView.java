...
import static com.moudle.ijkplayer.StringUtils.generateTime;
...
public class IjkPlayerView extends FrameLayout implements View.OnClickListener {
    ...
    private MarqueeTextView mTvTitleVer;
    // 是否启用判断wifi
    ...
    private void _initView(Context context) {
        ...
        mTvTitleVer = (MarqueeTextView) findViewById(R.id.tv_title_vertical);
        ...
        mWindowTopBar = (LinearLayout) findViewById(R.id.window_top_bar);
        ...
        isCheckWIFI = PublicPreferencesUtils.getBoolean(context, ManagerUtils.IS_USEWIFI);
    }
...
//        }
     ...
     * @return
     */
    public IjkPlayerView init(Context context) {
        ...
        if (mVideoView.isPlaying()) {
            ...
            if (!isCheckWIFI)
                ...
                if (ManagerUtils.isWifiEnabled(context)) start();
                    ...
                    ManagerUtils.createTwoDialog(context, "您当前未使用WIFI网络，是否继续观看", "继续", "取消", new DialogInterface.OnClickListener() {
                        @Override
